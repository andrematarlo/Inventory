...
<div class="modal fade" 
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form action="{{ route('items.update', ['item' => $item->ItemId]) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('ClassificationId') is-invalid @enderror" 
                                id="ClassificationId{{ $item->ItemId }}" 
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('UnitOfMeasureId') is-invalid @enderror" 
                                id="UnitOfMeasureId{{ $item->ItemId }}" 
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" 
                               class="form-control @error('ReorderPoint') is-invalid @enderror" 
                               id="ReorderPoint{{ $item->ItemId }}" 
                               name="ReorderPoint" 
                               value="{{ old('ReorderPoint', $item->ReorderPoint) }}" 
                               min="0" 
                               required>
                        @error('ReorderPoint')
                            <div class="invalid-feedback">{{ $message }}</div>
